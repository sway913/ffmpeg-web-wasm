...
extern "C" void log_callback2(void *avcl, int level, const char *fmt, va_list vl)
{
  char log[1024] = {0};
...
public:
  Decoder2(emscripten::val jsUpstream);
  ~Decoder2();
...
{
  emscripten::class_<Decoder2>("Decoder2")
      .constructor<emscripten::val>()
      .function("next", &Decoder2::Next);
}

int DecoderReadPacket2(void *opaque, uint8_t *buf, int buf_size)
{
  Decoder2 *pDecoder = (Decoder2 *)opaque;
...
DECODER_ERROR Decoder2::Initialize()
{
  if (m_bInitlized)
  {
  ...
  av_log_set_callback(log_callback2);
  ...
  AVIOContext *pIoCtx = avio_alloc_context(pReadBuffer, nBufferSize, 0, (void *)this, DecoderReadPacket2, NULL, NULL);
  if (pIoCtx == NULL)
  {
    return DECODER_ERROR::AV_ERROR;
  }
...
int Decoder2::ReadPacket(void *opaque, uint8_t *buf, int buf_size)
{
